#!/bin/bash
# This came from TPOT project
# Modified for BlackRain
...
if [ "$myNFQCHECK" == "honeytrap" ];
  then
    /usr/sbin/iptables -w -A INPUT -s 127.0.0.1 -j ACCEPT
    /usr/sbin/iptables -w -A INPUT -d 127.0.0.1 -j ACCEPT

    for myPORT in $myRULESPORTS; do
      /usr/sbin/iptables -w -A INPUT -p tcp --dport $myPORT -j ACCEPT
    done

    /usr/sbin/iptables -w -A INPUT -p tcp --syn -m state --state NEW -j NFQUEUE
fi

### Setting up iptables-legacy rules for glutton
#if [ "$myNFQCHECK" == "glutton" ];
#  then
#    /usr/sbin/iptables -w -t raw -A PREROUTING -s 127.0.0.1 -j ACCEPT
#    /usr/sbin/iptables -w -t raw -A PREROUTING -d 127.0.0.1 -j ACCEPT
#
#    for myPORT in $myRULESPORTS; do
#      /usr/sbin/iptables -w -t raw -A PREROUTING -p tcp --dport $myPORT -j ACCEPT
#    done
...
if [ "$myNFQCHECK" == "honeytrap" ];
  then
    /usr/sbin/iptables -w -D INPUT -s 127.0.0.1 -j ACCEPT
    /usr/sbin/iptables -w -D INPUT -d 127.0.0.1 -j ACCEPT

    for myPORT in $myRULESPORTS; do
      /usr/sbin/iptables -w -D INPUT -p tcp --dport $myPORT -j ACCEPT
    done

    /usr/sbin/iptables -w -D INPUT -p tcp --syn -m state --state NEW -j NFQUEUE
fi
