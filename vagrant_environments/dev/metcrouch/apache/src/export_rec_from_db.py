...
def main():
    ...
    sql = "SELECT id, pressure, wind_deg, temp, rain, humidity, wind_speed FROM " + table_name.__str__() + " WHERE location = '" + location + "' AND id >= " + low_id.__str__() + " AND id <= " + high_id.__str__()
    print("SQL : " + sql)
    ...
    header = "id\tpressure\twind_deg\trainx100\twind_speedx100\twind_deg_diff\ttemp\thumidity\n"
    fp_out.write(header)
    ...
    for row in records:
        ...
        humidity = row[5]
        wind_speed = row[6] * 8        # multiplication to make visible in graph only

        wind_deg_diff = wind_deg - last_wind_deg
        csv_rec = id.__str__() + "\t" + pressure.__str__() + "\t" + wind_deg.__str__() + "\t" + rain.__str__() + "\t" + wind_speed.__str__() + "\t" + wind_deg_diff.__str__() + "\t" + temp.__str__() + "\t" + humidity.__str__() + "\n"
        fp_out.write(csv_rec)
        last_wind_deg = wind_deg
        print(csv_rec)
